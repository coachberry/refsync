...
// ── Inline SVG icons (keeps sidebar self-contained) ───────────────────────────
...
const NAV_CONFIG = {
  official: [
    ...
    { section: 'Communication', items: [
      { to: '/official/inbox',   label: 'Inbox',    icon: MsgIcon, badge: 'inbox' },
  ...
  scheduler: [
    ...
    { section: 'Management', items: [
      ...
      { to: '/scheduler/messages', label: 'Messages',  icon: MsgIcon, badge: 'messages' },
  ...
  director: [
    ...
    { section: 'Communication', items: [
      { to: '/director/messages', label: 'Messages', icon: MsgIcon, badge: 'messages' },
    ]},
    { section: 'Account', items: [
...
const ROLE_META = {
  official:  { label: 'Official / Referee', icon: '🏒', color: 'var(--red)' },
  scheduler: { label: 'Scheduler',          icon: '📋', color: 'var(--teal)' },
  director:  { label: 'Tournament Director',icon: '🏆', color: 'var(--blue)' },
}
    ...
    <>
      ...
      <aside className={[styles.sidebar, isOpen ? styles.open : ''].join(' ')}>
        ...
        <div className={styles.activeRole} style={{ '--role-color': ROLE_META[activeRole]?.color }}>
          ...
          <span className={styles.activeRoleLabel}>{ROLE_META[activeRole]?.label}</span>
        </div>
        ...
        {otherRoles.length > 0 && (
          <div className={styles.roleSwitcher}>
            <span className={styles.sectionLabel}>Switch Role</span>
            {otherRoles.map(role => (
              <button
                key={role}
